...
You are the first of two stages. Everything you write is checked afterwards by a separate audit that rejects any probe revealing an outcome, so **generate generously and let the audit do the rejecting**. A borderline moment that the audit throws out costs nothing; a safe moment you never wrote is lost for good.

- Aim for 4 to 6 candidates per episode. Mine the whole summary: settings, errands, objects, side characters' routines, running gags, the texture around the big events.
- A moment adjacent to a turning point is fine as long as the sentence itself does not state the outcome. "Walt buys a gun" is a plan, not an outcome: include it. "Gus kills Victor" is an outcome: leave it out.
- Return zero only when the summary genuinely contains no situation at all, which is rare. A summary that is all turning points still has rooms, cars, meals, weather, jobs and journeys around them.

## Output

Return up to 6 candidate probes for the target episode, best first. For each: the probe text, a specificity score from 1 (could be several episodes) to 5 (unmistakably this one), the character names used, and the verbatim phrase from the summary it is grounded in.
